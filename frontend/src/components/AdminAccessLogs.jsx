...
import {
    Shield, Search, Filter, Calendar, ChevronDown, CheckCircle, XCircle,
    MoreHorizontal, Download, RefreshCw, AlertTriangle
} from 'lucide-react';
...
export default function AdminAccessLogs() {
    ...
    useEffect(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(log => {
                const patientName = `${log.patient?.first_name} ${log.patient?.last_name}`.toLowerCase();
                const workerInfo = `${log.healthcare_worker?.job_title} ${log.healthcare_worker?.license_number}`.toLowerCase();
                const facility = log.healthcare_worker?.healthcare_facility?.name?.toLowerCase();
                return patientName.includes(q) || workerInfo.includes(q) || (facility && facility.includes(q));
            });
        }
    ...
    const displayLogs = expanded ? filteredLogs : filteredLogs.slice(0, INITIAL_LIMIT);
    ...
    if (error) {
        return (
            <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg flex items-center gap-2">
                ...
                {error}
                <button onClick={fetchLogs} className="ml-auto text-sm underline hover:text-red-900">Retry</button>
            </div>
        );
    }
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {displayLogs.length > 0 ? (
                            displayLogs.map(log => (
                                <tr key={log.log_id} className="hover:bg-gray-50 transition-colors group">
                                    ...
                                    <td className="px-5 py-3">
                                        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold capitalize
                                            ${log.action === 'VIEW' ? 'bg-blue-100 text-blue-700' :
                                                log.action === 'EDIT' ? 'bg-amber-100 text-amber-700' :
                                    ...
                                    <td className="px-5 py-3 text-gray-500 max-w-[200px] truncate" title={log.reason}>
                                        {log.reason}
                                    </td>
                                </tr>
                            ))
            ...
            <div className="px-5 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center text-sm">
                <span className="text-gray-500">
                    Showing <span className="font-medium text-gray-900">{Math.min(displayLogs.length, filteredLogs.length)}</span> of <span className="font-medium text-gray-900">{filteredLogs.length}</span> logs
                    {allLogs.length !== filteredLogs.length && <span className="text-xs ml-1">(filtered from {allLogs.length})</span>}
                </span>
